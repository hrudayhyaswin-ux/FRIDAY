...
class LazySpeechToText:
    ...
    def _load_model(self):
        ...
        try:
            ...
            print(
                f"[Whisper STT] Failed loading model on {device}, falling back to CPU: {e}",
                flush=True,
            )
            self.model = whisper.load_model(self.model_name, device="cpu")
            print("[Whisper STT] Model loaded successfully on CPU!", flush=True)
    ...
    def transcribe(self, audio_file_path: str) -> str:
        try:
            ...
            print(
                f"[Whisper STT] Transcribing: {audio_file_path} | Size: {file_size} bytes",
                flush=True,
            )
