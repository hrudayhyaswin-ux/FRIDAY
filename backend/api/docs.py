...
from documents.service import delete_document, process_document
...
async def upload_document(file: UploadFile = File(...)) -> dict:  # noqa: B008
    ...
    try:
        ...
        doc_id = process_document(file_path, file.filename or "unknown")

        # Query metadata of document to return
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, filename, file_type, file_size, status, created_at FROM documents WHERE id = ?",
            (doc_id,),
        )
        row = cursor.fetchone()
...
def get_document_details(id: str):
    ...
    try:
        # Fetch document
        cursor.execute(
            "SELECT id, filename, file_type, file_size, status, metadata, created_at FROM documents WHERE id = ?",
            (id,),
        )
        doc = cursor.fetchone()
        if not doc:
        ...
        if knowledge_row:
            try:
                extracted_knowledge = {
                    ...
                    "structured_data": json.loads(knowledge_row["structured_data"])
                    if knowledge_row["structured_data"]
                    else {},
                }
...
def query_documents(request: QueryRequest):
    ...
    results = query_rag(request.query, k=request.k or 5)
...
def clear_documents():
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "All documents and vector indices successfully wiped.",
        }
    except Exception as e:
