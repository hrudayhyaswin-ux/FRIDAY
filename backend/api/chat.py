...
class ChatRequest(BaseModel):
    model: str | None = Field(None, description="The model name to use. Falls back to default if not provided.")
...
def chat(request: ChatRequest):
    ...
    system_prompt = (
        ...
        "ABSOLUTE RULE: Reply in 1-2 sentences MAXIMUM. No lists. No bullet points. "
        "No markdown. No explanations unless asked. "
            ...
            if last_msg.role == "user":
                ...
                if matches:
                    rag_context = (
                        ...
                        + "\n\n".join(
                            [
                                f"[File: {m['filename']}] (Method: {m['method']}, Score: {m['score']:.2f})\n{m['text']}"
                                for m in matches
                            ]
                        )
    ...
    for i, msg in enumerate(request.messages):
        ...
        if i == len(request.messages) - 1 and msg.role == "user":
            ...
            content += "\n\n[SYSTEM OVERRIDE: You MUST reply in 1-2 sentences only. No lists. Plain text only.]"

        formatted_messages.append({"role": msg.role, "content": content})

    return StreamingResponse(
        llm_client.chat_stream(model, formatted_messages),
