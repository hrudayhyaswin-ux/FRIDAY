...
def extract_entities(text: str, max_chars: int = 4000) -> list[dict[str, Any]]:
    ...
    prompt = (
        "Extract key named entities from the following text. Categorize them into "
        "'Person', 'Organization', 'Location', or 'Date'. "
        "Return ONLY a JSON list of objects, each with 'name' and 'type' keys. "
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a JSON-only entity extraction service. Output valid JSON list only, no explanation.",
        },
